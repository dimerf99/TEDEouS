...
from tedeous.loss_landscape.losses_of_plot import loss_grid_to_trajectory, rec_loss_function, loss_anchor
from tedeous.loss_landscape.trajectories_data import get_trajectory_dataloader, get_anchor_dataloader, \
    get_predefined_values
from tedeous.loss_landscape.loss_landscape_utils import get_files, get_gridpoint_and_trajectory_datasets, \
...
class VisualizationModel:
    ...
    def __init__(self,
        ...
        self.loss_dict = {
            ...
            'wellspacedtrajectory': {'official_name': "Well-spaced-trajectory loss",
    ...
    def get_files_and_compile_train_mode(self, batch_size: int = 32, state_dicts: dict = None):
        ...
        if state_dicts is None:
            ...
            rec_data_loader, transform = get_trajectory_dataloader(pt_files, batch_size,
                                                                   self.path_to_plot_model_directory)
        ...
        if self.isEnabled('gridscaling'):
            self.loss_dict['gridscaling']['dataloader'] = get_gridpoint_and_trajectory_datasets(
                pt_files,
                self.path_to_plot_model_directory,
                self.grid_step,
            ...
            self.l_max_inputspace = torch.sqrt(
            ...
            if callbacks.callbacks[0].stop_training is False:
                ...
                for batch_idx in range(max_batches):
                    ...
                    if self.isEnabled('lastzero'):
                        ...
                        last_coordinate = z[-1, :]
                        loss_zero = torch.nn.functional.mse_loss(10 * last_coordinate,
                                                                 torch.zeros_like(last_coordinate))
                    ...
                    if self.isEnabled('polars'):
                        ...
                        loss_zero = torch.nn.functional.mse_loss(10 * last_coordinate,
                                                                 10 * 0.8 * torch.ones_like(last_coordinate))
                        loss_zero2 = torch.nn.functional.mse_loss(10 * first_coordinate,
                                                                  10 * -0.8 * torch.ones_like(first_coordinate))
                    ...
                    if self.isEnabled('gridscaling'):
                        ...
                        losses['gridscaling'] = loss_grid_to_trajectory(self.AE_model, data_grid_latent,
                                                                        data_trajectory, self.l_max_inputspace,
                                                                        epoch=self.epoch,
