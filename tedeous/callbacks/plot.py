...
class Plots(Callback):
    ...
    def _print_nn(self):
        ...
        if nvars_model is None and self.method == 'PINN':
            ...
            nvars_model = self.net.out_features
        ...
        for i in range(nvars_model):
            if nparams == 1:
                ...
                if self.method == 'PINN':
                    x = self.grid[:, 0].detach().cpu().numpy()
                    y = self.grid[:, 1].detach().cpu().numpy()
                    z = self.net(self.grid)[:, i].detach().cpu().numpy()

                    ax1.plot_trisurf(x, y, z, cmap=cm.jet, linewidth=0.2, alpha=1)
                    ...
                    s_pred = self.net(torch.tensor(u_test, dtype=torch.float32),
                    ...
                    x = self.grid[:, 0].detach().cpu().numpy().reshape(-1)
                    y = self.grid[:, 1].detach().cpu().numpy().reshape(-1)
                    z = s_pred.detach().cpu().numpy()
