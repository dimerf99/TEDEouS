...
def wave_experiment(grid_res):
    ...
    # PINN without special init: iter = 13500, loss = 1.4e-2, time = 8 min

    for m in net.modules():
        if isinstance(m, torch.nn.Linear):
            torch.nn.init.xavier_normal_(m.weight)  # iter = 1e+4, loss = 5e-4, time = 6 min
            # torch.nn.init.kaiming_normal_(m.weight)  # iter = 1e+4, loss = 1.4e-1
            # torch.nn.init.xavier_uniform_(m.weight)  # iter = 1e+4, loss = 1.6e-3
            # torch.nn.init.kaiming_uniform_(m.weight)  # iter = 1e+4, loss = 9.1e-3
