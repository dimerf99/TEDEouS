...
def func(grid):
    ...
    for i in range(1, 100):
        sln += 8 * np.exp(-1 / 4 * np.pi ** 2 * t * (2 * i - 1) ** 2) * (
                (-1) ** i + 250 * np.pi * (1 - 2 * i)) * np.sin(1 / 2 * np.pi * x * (2 * i - 1)) * np.sin(
            1 / 2 * np.pi * y * (2 * i - 1)) / (np.pi - 2 * np.pi * i) ** 2
    return sln
...
def heat_experiment(grid_res, CACHE):
    ...
    boundaries.dirichlet({'x': 0, 'y': [0, 1], 't': [0, 1]}, value=500)

    # Boundary conditions at y=0
    boundaries.dirichlet({'x': [0, 1], 'y': 0, 't': [0, 1]}, value=500)
